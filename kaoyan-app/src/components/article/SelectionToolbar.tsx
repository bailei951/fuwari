// 选区浮动工具条：选中文本后浮起，提供「查词」「翻译」「批注」三个动作
// 查词：仅对单个英文单词可用（本地牛津词典 + 在线词典）
// 翻译：任意长度文本可用（短语 / 长句 / 中文 → 自动判定方向，多源容错）
// 批注：任意 2 字符以上选区可用（阅读文章）

import { BookOpen, Languages, Highlighter } from 'lucide-react'
...
export default function SelectionToolbar({
  ...
  // 水平居中于选区
  const toolbarWidth = onAnnotate ? 196 : 136
  ...
  return (
    ...
    >
      <button
        ...
        title={canLookup ? '查词（本地词典）' : '仅单个英文单词可查词'}
      >
      ...
      <button
        ...
        className="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs hover:bg-ochre-dark rounded-sm transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
        title={canTranslate ? '翻译选中文本' : '选区过短'}
      >
